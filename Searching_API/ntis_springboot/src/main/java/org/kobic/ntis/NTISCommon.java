...
public class NTISCommon {
	...
	public static String url_head = "http://fast.ntis.go.kr/apis/service/search/project?userKey=821614f489a22930b4971025776fb4b2e2b0cc3d&query="; // url 앞부분																																					
	...
	public JSONArray getNtisXmlDataFile(String input) {
		...
		for (int i = 0; i < howMany; i++) {
			/*
			 * API 문서대로 필드:(키워드) 형태를 만들어주기위한 과정 + 필드값이 적절한 값인지 isMember 함수를 사용해서 검사하고,
			 *  적절하지 않은 필드는 디폴트값인 bi로 치환한다.
			 */
	...
	private JSONArray makeJSON(List<ResultModel> modelList) {
		...
		for (ResultModel m : modelList) {
			...
			element.put("Manager", m.getManager());
			element.put("LeadAgency", m.getLeadAgency());
			element.put("ManageAgency", m.getManageAgency());
			element.put("Ministry", m.getMinistry());
			element.put("Year", m.getProjectYear());
			element.put("StartDate", m.getStartDate());
			element.put("EndDate", m.getEndDate());
